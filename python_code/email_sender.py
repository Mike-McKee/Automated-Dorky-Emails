...
from select_random_item import email_item
import EMAILS
...
def gmail_send_message():
    ...
    message_contents = email_item()
    title = message_contents[1]
    date = message_contents[4]
    summary = message_contents[2]
    link = message_contents[3]

    # flow = InstalledAppFlow.from_client_secrets_file('GOOGLE_SECRETS/credentials.json', SCOPES)
    # creds = flow.run_local_server()
    SCOPES = ['https://www.googleapis.com/auth/gmail.compose']
    creds = Credentials.from_authorized_user_file('GOOGLE_SECRETS/token.json', SCOPES)
